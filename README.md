...
| `--date` | 昨天 | 补推指定日期的日报（东八区）。窗口锚定当天 0:00~24:00 整个自然日，同一天推几次内容都一样 |
...
**组织元仓库排除**：`.github` 是 GitHub 的组织级特殊仓库，存 profile README、issue 模板与 `github-metrics` 定时任务自动生成的统计图表。它没有真人贡献（实测一年 5583 条提交全部是 bot、`author_login` 为空），却因那个定时任务 `pushed_at` 长期最新而稳居活跃仓库榜首，让外部读者困惑。名单写在 `contributors/repos.py` 的 `EXCLUDED_REPOS`，优先级高于 `--repos` 白名单——显式指定也不会纳入。
...
两个 GraphQL 查询都按 `CREATED_AT DESC` 排序，所以窗口内的数据全在最前面
几页。一旦某页最早的一条都早于窗口起点，后面只会更早，翻回来也会被窗口
过滤丢掉——此时直接收手。

判据是 `createdAt`，但一个老 PR 可能在窗口内才被合并或评论，卡在 `since`
就停会漏掉这类事件。所以要往前多翻一段缓冲期，由 `api_stats.EARLY_STOP_BUFFER`
控制，默认 90 天。调大更保险、调小更快，只影响速度与漏判风险的取舍。

abacus-develop（4945 个 PR + 2691 个 issue）实测：

| | 请求数 | 说明 |
|---|---|---|
| 早停前 | 306 次 | PR 198 页 + Issue 108 页，一路翻到底 |
| 早停后 | 63 次 | PR 45 页 + Issue 18 页 |

这不只是省额度。连打 306 次请求要好几分钟，期间任何一次连接被掐断整个仓库
就判失败，重试还得从第 1 页重来——2026-08-14 该仓库连挂三天正是这么来的。
把暴露时间缩掉八成，比多加几轮重试有效得多。

对大部分仓库这个改动不触发（总页数本来就在窗口内），也不会让任何仓库变慢：
早停只可能提前结束循环。

关于整批耗时：**端到端时长尚未在正常网络下实测**，下文各处仍沿用早停前的
25 分钟基准。已确证的只有请求数 306→63；换算成时间要看单次请求延迟，且
整批耗时里 git fetch 与其余 37 个仓库占大头，所以整体降幅远小于 abacus-develop
单仓库的比例。等网络平稳时跑一次 `--fetch` 再回来更新这些数字。
...
日报的 PR 按**在干什么**分区（新功能 / 修复 / 重构 / 性能 / 测试 / 文档 /
工程 / 回滚 / 其他），而不是按合并/新建分区。读日报的是运营，他们要的是
「社区最近在推新功能还是在修 bug」，不是 PR 的生命周期状态。新功能排最前（运营
写推文的素材主要来自这里），其他排最后，空类别不渲染，每类最多 5 条。
合并/新建信息移到条目行首的 `[合并]` / `[新建]` 标记，不丢。

周月报则在「活跃仓库」后多一个「PR 动向」分区，给仓库 × 意图矩阵。
注意它与「活跃仓库」**口径不同**：那个数全部事件（commit+PR+issue）用「次活动」，
这个只数 PR 用「个 PR」，同一仓库两边的数字和排名都可能不一样，这不是 bug。

分类只用 PR。实测近 90 天 conventional commits 前缀覆盖率：PR 75%、
commit 44%、issue 9%；而且 commit 是过程不是意图（一个 PR 里混着大量
"fix typo"、"address review"），按它分类会让改错别字和重写求解器占同样的权重。

分隔符除冒号外也认空格和斜杠（`Fix module_hs`、`perf/Optimize xxx`）——
只认冒号的话识别率会从 75% 掉到 65%。当前「其他」占 25%，集中在
abacus-develop（60% 规范度，却是最活跃仓库）等几个仓库，待后续用 LLM 补齐，
进展与规划见 `docs/运维/卡片语义化运维日志.md`。
...
```
schtasks /create /tn "DM日报-抓取" /tr "C:\...\daily_report.bat --fetch"   /sc daily   /st 18:00
...
**抓取必须在当晚 24:00 之前跑完。** 日报窗口锚定当天 0:00~24:00 整个自然日，
24:00 之后入库的事件落进第二天的窗口。18:00 起跑留约 6 小时余量——
全量抓取实测 24～28 分钟，剩下的余量给失败重试。18:00~24:00 之间如果又有
新增，那批要等第二天 18:00 才抓到，届时天然算进"昨天"的窗口，不丢也不重复，
只是不会精确到发生的当天。

不用 Windows 计划任务、改由外部调度器或 agent 触发也一样：约束只有
"抓取在当晚 24:00 前跑完"这一条，推送时刻随意，同一天推几次内容都相同。
窗口锚点写在 `contributors/notify/period.py` 的 `DAILY_ANCHOR_HOUR`，
改它要同步改 `tests/test_notify.py` 里锁定窗口行为的四个断言。

抓取与推送分成两个任务是因为抓取耗时不可控（受当天新提交量与网络影响），
而推送只读库、秒级完成，可以准点。若不在乎准点，用 `--daily` 一个任务
串联即可，但要保证它在当晚 24:00 前跑完。

必须以当前用户身份运行——token 来自 `gh` 的 keyring，换用户取不到。
机器关机或休眠时任务不会跑，这是本机方案的固有限制。

飞书对单个机器人限流 100 次/分钟、5 次/秒，官方文档建议避开整点、半点。
故推送时刻取 10:03 而非 10:00；抓取那步不发消息，
没有这个约束。
...
连续 3 次抓取失败的仓库会升级为单独一行告警。分开写是因为语义不同——
「累计到明天」对这类仓库不再成立，必须有人去看。判定只看真正跑了采集的
运行，拆分模式的推送步骤不参与计数。

告警文案按最近一次的失败原因分流，不写死猜测：

| 原因类别 | 判据示例 | 文案 |
|---|---|---|
| permanent | 404、401/403、Could not resolve to a Repository | 疑似改名、删除或权限变更，请检查仓库 |
| network | RemoteDisconnected、Connection reset、超时、SSLEOFError | 均为网络错误，仓库多半正常，请检查网络或抓取时段 |
| unknown | 都不匹配 | 原因未能识别，请查日志 |

分类顺序是先永久后网络：一次请求可能既超时又返回 404，那种情况按永久处理，
因为重试再多也没用。认不出来时用中性措辞，宁可不给方向也不给错方向。

这个分流是 2026-08-14 补的。当天 abacus-develop 连挂三次触发告警，而原文案
写死「可能是改名、删除或权限变更」——实际原因是 RemoteDisconnected，仓库
完全正常，三个猜测一个都不中，把排查引向了错误方向。
...
日报窗口锚定当天 0:00~24:00（东八区整个自然日）：8-13 的日报固定是
8-13 0:00 ~ 8-13 24:00，**由日期决定，与实际推送时刻无关**。所以抓取与
推送可以分开跑，同一天推几次内容都一样，窗口内的任何补跑都会自动计入。

不指定 `--date` 时默认取**昨天**：锚点在 0 点，今天的窗口要到今晚 24:00 才
关闭，此刻推会推到一个尚未闭合的区间。取昨天后，同一自然日内任意时刻推送
内容都一致，过了午夜 0 点才翻到下一天。

```bash
python -m contributors --only-notify              # 推昨天（默认）的日报
...
事件库 `data/events.db` 不进主线 git 历史（`.gitignore` 里排除），因为它持续
增长，混进代码提交会让仓库越 clone 越慢。但服务器部署后它是唯一记录「第几次
运行首次看到」这个信息的地方，值得单独备份。

方案是给这个仓库开一条独立的 `backup-data` 孤儿分支（跟 `master` 没有共同
历史，不会互相污染），服务器每天定时把 `data/events.db` 的快照 commit 上去。
**明确只备份这一个文件**——`.env`（飞书 webhook 密钥）永远不进任何 git 分支。

要拿这份数据：

```bash
# 拉最近一次快照，日常调试够用
git clone --depth=1 -b backup-data <仓库地址> contributors-data
```

已有本地 clone 时用 `git fetch --depth=1` + `git reset --hard
origin/backup-data` 而不是 `git pull`——这条分支每天一个快照 commit，不加
`--depth=1` 会把历史上所有快照的完整数据库 blob 都拉下来，越攒越大。

要"刚刚发生"的最新状态（备份是定时任务，有延迟），改用 `scp` 直连服务器现取，
配套的服务器迁移与运维细节见六小瓜项目的 `docs/运维/迁移到服务器.md`（同一次
迁移里两个项目一起搬，方案通用）。
