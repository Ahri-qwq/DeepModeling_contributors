...
| `--date` | 今天 | 补推指定日期的日报（东八区）。窗口锚定当天 11:00，同一天推几次内容都一样 |
...
**抓取必须在 11:00 之前跑完。** 日报窗口锚定当天 11:00，11:00 之后入库的
事件落进第二天的窗口，当天日报会是空的。07:00 起跑留约 4 小时余量——
全量抓取实测 24～28 分钟，剩下的余量给失败重试。

不用 Windows 计划任务、改由外部调度器或 agent 触发也一样：约束只有
"抓取在 11:00 前跑完"这一条，推送时刻随意，同一天推几次内容都相同。
...
日报窗口锚定当天 11:00（东八区）：8-13 的日报固定是 8-12 11:00 ~ 8-13 11:00，
