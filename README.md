...
| `--date` | 昨天 | 补推指定日期的日报（东八区）。窗口锚定当天 20:00，同一天推几次内容都一样 |
...
**抓取必须在 20:00 之前跑完。** 日报窗口锚定当天 20:00，20:00 之后入库的
事件落进第二天的窗口，当天日报会是空的。18:00 起跑留约 2 小时余量——
全量抓取实测 24～28 分钟，剩下的余量给失败重试。

不用 Windows 计划任务、改由外部调度器或 agent 触发也一样：约束只有
"抓取在 20:00 前跑完"这一条，推送时刻随意，同一天推几次内容都相同。
...
日报窗口锚定当天 20:00（东八区）：8-13 的日报固定是 8-12 20:00 ~ 8-13 20:00，
**由日期决定，与实际推送时刻无关**。所以抓取与推送可以分开跑，同一天推几次
内容都一样，窗口内的任何补跑都会自动计入。

不指定 `--date` 时默认取**昨天**：锚点在 20:00，今天的窗口要到今晚 20:00 才
关闭，早上推会推到一个尚未闭合、且必然为空的区间——昨晚抓的那批落在昨天的
窗口里。取昨天后，同一自然日内任意时刻推送内容都一致，过了午夜 0 点才翻到
下一天。
