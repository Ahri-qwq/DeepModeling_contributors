...
#
# 建议的 crontab 时间点（完整跑一遍约 25 分钟，留足余量）：
#   18:00  daily_report.sh --fetch
#   10:01  daily_report.sh --push
# fetch 必须在 24:00 窗口关闭前跑完，否则事件会落进下一天的窗口，
# 当天报告就会漏掉这部分数据（不是丢失，是滚入明天的报告，且不会
# 重复计数）。18:00 留了约 6 小时的重试余量。飞书文档建议避开整点/
# 半点（限流 100/分钟，5/秒），所以是 10:01 而不是 10:00。
# fetch 步骤没有这个限制——它不发送任何东西。
# ============================================================
